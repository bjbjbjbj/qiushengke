...
namespace App\Http\Controllers\TTZB;
                ...
                if (strlen($timeStr) > 0 && strlen($teamStr) > 0 && strlen($liveStr) > 0){
                    ...
                    if (count($teamStr) == 2) {
                        ...
                        if (isset($match)){
                            ...
                            MatchLiveChannel::saveSpiderChannel($match->id,MatchLive::kSportFootball,MatchLiveChannel::kTypeTTZB,$liveStr,12,MatchLiveChannel::kPlatformPC,MatchLiveChannel::kPlayerIFrame,'体育直播', $show, $private, MatchLiveChannel::kUseAikq);
                            echo 'match ' . $host . ' vs ' .$away . ' 频道 '.$liveStr.'</br>';
                        }
                        else{
                            $alise = LiveAlias::getAliasByName($host,LiveAlias::kTypeTeam,LiveAlias::kFromTTZB);
                            if (is_null($alise)){
                                $alise = LiaogouAlias::where('from',LiaogouAlias::kFromTTZB)
                ...
                if (strlen($timeStr) > 0 && strlen($teamStr) > 0 && strlen($liveStr) > 0){
                    ...
                    if (count($teamStr) == 2) {
                        ...
                        if (isset($match)){
                            ...
                            $show = $isPrivate ? MatchLiveChannel::kHide : MatchLiveChannel::kShow;
                            $private = $isPrivate ? MatchLiveChannel::kPrivate : MatchLiveChannel::kNotPrivate;
                            MatchLiveChannel::saveSpiderChannel($match->id,MatchLive::kSportBasketball,MatchLiveChannel::kTypeTTZB,$liveStr,12,MatchLiveChannel::kPlatformPC,MatchLiveChannel::kPlayerIFrame,'体育直播', $show, $private, MatchLiveChannel::kUseAikq);
                            echo 'match ' . $host . ' vs ' .$away . ' 频道 '.$liveStr.'</br>';
                        }
                        else{

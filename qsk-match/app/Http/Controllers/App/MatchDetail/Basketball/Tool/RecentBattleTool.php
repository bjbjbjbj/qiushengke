 ...
 * Time: 14:59
 */
namespace App\Http\Controllers\App\MatchDetail\Basketball\Tool;
        ...
        $allMatches = BasketMatch::query()
            ->leftJoin($matchDBStringPre.'odds as goal', function ($join) use ($cid,$matchDBStringPre) {
                ...
                $join->where('goal.cid', $cid)
                    ->where('goal.type', 4);
            })->leftJoin($matchDBStringPre.'leagues', 'matches.lid', '=', $matchDBStringPre.'leagues.id')
...
//            ->where('time', '>=', $lastTime)
    ...
    public function recentBaseBattle($outMatch, $cid = Odd::default_banker_id, $count = 20) {
        ...
        $allMatches = BasketMatch::query()
            ...
            })->leftJoin($matchDBStringPre.'leagues', $matchDBStringPre.'matches.lid', '=', $matchDBStringPre.'leagues.id')
            ->select($matchDBStringPre.'matches.*', $matchDBStringPre.'leagues.name as league',
    ...
    private function getBaseLastTime($match, $count) {
        $matchDBStringPre = 'basket_';
        $homeMatches = BasketMatch::where('hid', $match->hid)
            ...
            ->take($count)
            ->get();
        $awayMatches = BasketMatch::where('aid', $match->aid)
            ->where('time','<',$match->time)
            ->where('lid',$match->lid)
            ->where($matchDBStringPre.'matches.status',-1)
            ->orderby('time','desc')

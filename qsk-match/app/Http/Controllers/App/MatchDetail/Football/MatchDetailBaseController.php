...
class MatchDetailBaseController extends Controller {
    ...
    public function matchDetailBaseData($mid){
        //缓存
        $cacheSession = $mid.'_matchDetailBase';
        $match = Match::where('matches.id','=',$mid)
            ->leftjoin('leagues','lid', '=', 'leagues.id')
            ->select('matches.*', 'leagues.type as leagueType')
            ->first();
...
//        dump($sameOdd);
        ...
        //排名
        $resultRank = $this->matchBaseRank($match);
    ...
    private function matchBaseOdd($currentMatch, $tid){
        ...
        //总
        $matches = Match::where('lid',$currentMatch->lid)
            ->where('status',-1)
//            ->where('season',$currentMatch->season)
            ...
            ->join('odds as asia',function ($join) use($cid){
        ...
        foreach ($matches as $match){
            ...
            if (isset($match->ouup2) && isset($match->oumiddle2) && isset($match->oudown2)){
                $countEu++;
                $sixEu[] = OddCalculateTool::getMatchSizeOddResult($match->hscore, $match->ascore, $match->oumiddle2);
            }
            else{
                $countEu++;
                $sixEu[] = -1;
...
//                dump($match->hname .' '.$match->hscore.' '.$match->aname .' '.$match->ascore);
    ...
    private function getMatchScheduleWithTid($match, $tid, $name){
        $matches = Match::where(function ($q) use($match,$tid,$name){
    ...
    private function matchBaseRecentlyBattleWithIsHost($match, $isHost){
        ...
        $result['all'] = $matches;
        //同主客
        $matches = Match::where($isHost?'hid':'aid',$tid)
            ->leftjoin('leagues','lid', '=', 'leagues.id')
            ->leftjoin('odds as corner',function ($join) use($cid){
                $join->on('matches.id', '=', 'corner.mid');
                $join->where('corner.type','=',1);

        ...
        if (!is_null($this->nodes))
        {
            foreach ($this->nodes as $n)
            {
                if($child){
                    ...
                    if ($n->nodetype == HDOM_TYPE_TEXT) $ret .= $n->_[HDOM_INFO_TEXT];
                }
            }
...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    function __get($name) {
        ...
        switch ($name) {
            ...
            case 'plaintext': return $this->text(true);
            case 'plainroottext': return $this->text(false);

        ...
        foreach ($sDatas as $data) {
            if (str_contains($data, "=")) {
                ...
                if (preg_match("/(?<=\\[)(.*)(?=\\])/i", $mid, $temps) && $length > 4) {
                    ...
                    list($a1, $b1, $c1, $a2, $b2, $c2, $up, $middle, $down) = explode(",", $oddList[0], 9);
                    ...
                    list($a1, $b1, $c1, $a2, $b2, $c2, $up, $middle, $down) = explode(",", $oddList[2], 9);
                    $array['all'][2] = ['up'=>$up, 'middle'=>$middle, 'down'=>$down]; //大小球
                    list($a1, $b1, $c1, $a2, $b2, $c2, $up, $middle, $down) = explode(",", $oddList[1], 9);
                    $array['all'][3] = ['up'=>$up, 'middle'=>$middle, 'down'=>$down]; //欧赔

                    //半场
                    list($a1, $b1, $c1, $a2, $b2, $c2, $up, $middle, $down) = explode(",", $oddList[3], 9);
                    $array['half'][1] = ['up'=>$up, 'middle'=>$middle, 'down'=>$down]; //亚盘
                    list($a1, $b1, $c1, $a2, $b2, $c2, $up, $middle, $down) = explode(",", $oddList[4], 9);
                    $array['half'][2] = ['up'=>$up, 'middle'=>$middle, 'down'=>$down]; //大小球
                    list($a1, $b1, $c1, $a2, $b2, $c2, $up, $middle, $down) = explode(",", $oddList[5], 9);
                    $array['half'][3] = ['up'=>$up, 'middle'=>$middle, 'down'=>$down]; //欧赔

                    $tmpMatch = MatchesAfter::where('win_id','=',$mid)->first();
                    if (isset($tmpMatch)) {
                        $l_mid = $tmpMatch['id'];
                        FileTool::putFileToLiveOdd($l_mid, $array,date('Ymd', strtotime($tmpMatch['time'])));
...
//                    list($up1, $middle1, $down1, $up2, $middle2, $down2, $other) = explode(",", $oddList[5], 7);
        ...
        FileTool::putFileToTotalLiveOdd($gunQiuArray);
    }
}